...
const ConnectFactorDialog = ({ isOpen, onClose, title, description }) => {
  ...
  const fetchData = async () => {
    ...
    await axios
      .get(
        `/api/manager/dasht/invoices?limit=${searchData?.limit || 10}&offset=${searchData?.offset || 0}&orderBy=${searchData?.orderBy || "Date"}&fromDate=${searchData?.fromDate || ""}&toDate=${searchData?.toDate || ""}`,
      )
      .then((response) => {
