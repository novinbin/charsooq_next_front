...
const Header = () => {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 mx-2 items-center rounded-lg bg-transparent bg-white px-4 shadow-lg transition-all duration-300 lg:h-[60px] lg:px-6",
        navbar && "",
      )}
    >
      <div className="my-auto flex h-14 w-full items-center justify-between pt-2.5">
        <div className="flex items-center gap-2">
          <Sheet>
            <SheetTrigger asChild>
              <Button
                size="icon"
                className="bg-red-primary hover:bg-red-dark shrink-0 text-foreground lg:hidden"
              >
            ...
            <SheetContent
              side="right"
              className="flex flex-col bg-white p-1 px-2"
            >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button className="mr-auto flex items-center justify-center gap-2 bg-primary text-white hover:bg-primary/90">
              <CircleUserRound size={24} />
              <div className="flex items-center gap-1">
                {/* <span>{userHook?.userData?.first_name_fa}</span>
              <span>{userHook?.userData?.last_name_fa}</span> */}
                <span>{farsiNumber(userHook?.userData?.name)}</span>
              </div>
          ...
          >
            ...
            <DropdownMenuItem className="w-full text-right">
              <Link className="w-full" href={routes.user.profile.details}>
                مشخصات کاربری
              </Link>
            </DropdownMenuItem>
      ...
      <UserInfo className="mb-2 flex justify-center md:hidden" />
    </header>
  );
};
