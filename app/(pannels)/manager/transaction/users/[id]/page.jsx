...
const UsersPage = ({ searchParams: { page }, params: { id } }) => {
  ...
  const fetchCities = async () => {
    setIsLoading(true);
    await axios
      .get(`/api/manager/transactions/users/${id}?page=${searchParams.get("page") || 1}&per_page=${searchParams.get("per_page") || 10}`)
      .then((response) => {
  ...
  return (
    <div className="px-0 lg:px-10">
      <DataTableHeader title={`تراکنش های  ${data?.data ? data?.data[0]?.user?.name   : "کاربر"}`} perPage={true} />
