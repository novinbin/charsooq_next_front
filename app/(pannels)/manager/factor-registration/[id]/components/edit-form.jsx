...
const EditForm = ({ data, params: { id, factorId } }) => {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      // total_price: separatePrice(data?.total_price),
  ...
  const onSubmit = async (values) => {
    ...
    await axios
      .post(`/api/manager/factor/${factorId}`, encodedFormData)
      .then((response) => {
        if (response.status === 200) {
          factorHook.setSelectedFactor(null);
          toast.success(<ToastSuccess text={"فاکتور با موفقیت تایید شد"} />);
          router.push(
            routes.manager["factor-registration"].factors.details(
              data?.user?.id,
              factorId,
            ),
          );
  ...
  const onSelect = () => {
    // setValue(
    //   "total_price",
    //   separatePrice(factorHook?.selectedFactor?.TotalPrice / 10),
    // );
  };
