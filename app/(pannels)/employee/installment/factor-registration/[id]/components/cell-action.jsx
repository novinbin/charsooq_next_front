...
const CellAction = ({ data }) => {
  ...
  const onDelete = async () => {
    try {
      ...
      const response = await axios.delete(`/api/employee/factor/${data.id}`);
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      ...
      <div className="flex flex-col gap-1">
        <Link
          href={routes.employee["factor-registration"].factors.details(
            params.id,
            data.id,
          )}
          className="w-full"
        ...
        <Link
          href={routes.employee["factor-registration"].factors.edit(
            params.id,
            data.id,
          )}
          className="w-full"
